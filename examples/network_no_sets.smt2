...
(declare-fun network.Network.connect (network.Network) (Tuple network.Host network.Router) :field)
...
(declare-fun network.connectRelation (network.Network) (Tuple network.Host network.Host) :properties (p0 p) )
...
(define-fun example1.Network1.config ( (n network.Network)) Bool 
  (and (= (mkTuple (example1.Network1.host1 n) (example1.Network1.router n) ) (network.Network.connect n))  
       (= (mkTuple (example1.Network1.host2 n) (example1.Network1.router n) ) (network.Network.connect n)) 
    )
