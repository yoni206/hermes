...
LA_DAG_var(TDAG DAG)
{
  ...
  if (!DAG_var_table->data[DAG])
    DAG_var_table->data[DAG] =
      simplex_hw_var_new(DAG_sort(DAG) == SORT_INTEGER ||
...
var_assoc_free(Tvar_assoc var_assoc)
{
  unsigned i;
  for (i = 0; i < var_assoc->n; i++)
    LAsigned_hw_clear(var_assoc->coefs[i]);
  free(var_assoc->vars);
...
LA_constraint_push2(TDAG DAG)
{
  ...
  if (stack_size(monoms) == 1)
    {
      ...
      if (LArational_hw_is_neg(monoms->data[0].coef))
        upper = !upper;
  ...
  for (i = 0; i < var_assoc->n; i++)
    {
      ...
      LArational_hw_mult_to_signed(var_assoc->coefs[i],
  ...
  if (var_assoc2)
  ...
  else
    {
      ...
      simplex_hw_constraint_push(var_assoc->n + 1,
 ...
 allocate:
  ...
  if (simplex_hw_var_integer(var))
      ...
      if (LArational_hw_is_int(indep_term)) /* implements the first table */
        {
          ...
          LAdelta_hw_set(&bounds->data[atom].delta2,
  ...
  else
    {
      ...
      LAdelta_hw_set_delta(&bounds->data[atom].delta,
                           upper?-strict:strict);
      LAdelta_hw_set_delta(&bounds->data[atom].delta2,
                           upper?(!strict):-(!strict));
    }
...
static inline void
LA_hw_notify_monom(const TLArational_hw rat,
                   const Tsimplex_var var, const Tpol pol)
{
  ...
  if (pol == POL_NEG)
    LArational_hw_neg(stack_top(monoms).coef);
...
static void LA_hw_notify_term(const TDAG, const Tpol, const TLArational_hw);
...
static inline bool
LA_hw_notify_term_top(const TDAG DAG, const Tpol pol, const TLArational_hw fact)
{
  TLArational_hw rat;
  unsigned i;
#if DEBUG_LA >= 2
...
/**
   ...
   \see LA_hw_notify_term, LA_hw_notify_term_top */
static void
LA_hw_notify_term(const TDAG DAG, const Tpol pol, const TLArational_hw fact)
{
  unsigned i;
  ...
  if (LA_hw_notify_term_top(DAG, pol, fact))
    return;
  LA_hw_notify_monom(fact, LA_DAG_var(DAG), pol);
...
void LA_hw_unate_propagation(void)
{
  unsigned i, *atom;
  size_t n;
  ...
  while (i+1 < n)
    {
      ...
      while(bounds->data[aj].var == v &&
            bounds->data[aj].upper)
        {
          ...
          if (LAdelta_hw_eq(&bounds->data[ai].delta,
      ...
      while (bounds->data[ai].upper && !bounds->data[aj].upper)
        {
          assert(bounds->data[ai].var == v);
          assert(bounds->data[aj].var == v);
          if (LAdelta_hw_less(&bounds->data[ai].delta,
                              &bounds->data[aj].delta))
            {
              /* (i : v <= b_i), (j : v >= b_j), b_i < b_j
                 |= (i) => !(j) -- lemma */
...
#if DEBUG_LA
  undo_push(LA_ASSERT);
  my_DAG_message("LA_hw_assert(%d) : %s%D%s\n", lit,
...
LA_hw_assert_eq(TDAG DAG0, TDAG DAG1)
{
  ...
  if (!svar)
    {
      ...
      svar = simplex_hw_var_new(DAG_sort(DAG0) == SORT_INTEGER &&
                                DAG_sort(DAG1) == SORT_INTEGER);
      simplex_hw_var_bounded(svar);
#if DEBUG_LA >= 2
      my_DAG_message("LA_hw_assert_eq %D = %D associated to %u\n", DAG0, DAG1, svar);
#endif
  ...
  if (simplex_hw_assert_lower_bound(svar, store.assert_eq.delta, 0) == UNSAT ||
...
cmp_simplex_assign(const void *P1, const void *P2)
{
  ...
  return simplex_hw_var_cmp(stack_get(DAG_var_table, DAG1),
      ...
      do
        {
          ...
          if (!simplex_hw_var_eq(sv_i, sv_j))
            break;
...
static Tstatus
LA_hw_solve_z_aux(unsigned d, unsigned m,
...
{
  size_t i;
  stats_unsigned("lemmas/bounds_hw",
                 "Bound lemmas generated by arith (HW version)",
                 "%9u", LA_lemma_n);
  if (!LA_disable_bbvsh)
    bb_data_done();
  if (LA_bound_ranking)
