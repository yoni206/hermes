...
row_free(Trow row)
{
  ...
  unsigned i;
  for (i = 0; i < row->alloc; ++i)
    LAsigned_xx_clear(row->data[i].a);
#endif
  stack_free(row);
}
  ...
  for (i = 0; i < modified_n; i++)
    {
      ...
      LAdelta_xx_set(&simplex_var[modified[i]].assign2,
  ...
  for (i = 0; i < modified_n; i++)
    {
      ...
      LAdelta_xx_set(&simplex_var[modified[i]].assign,
...
static inline void
simplex_var_update(Tsimplex_var i, TLAdelta_xx * D, TLAsigned_xx num, TLAsigned_xx den)
{
  int bound;
...
linear_expr_normalize_xx(Trow row)
{
  ...
  /* compute gcd */
  for (i = 1; LAsigned_xx_notone(store.normalize.tmp) && i < ROW_LEN(row); ++i)
    {
 ...
 const unsigned offset)
{
  ...
  while (it_k < ROW_LEN(*P) || it_i < ROW_LEN(L_i))
	  ...
	  (it_i < ROW_LEN(L_i) && ROW_VAR(L_i, it_i) < ROW_VAR(*P, it_k)))
	{
	  ...
	  LAsigned_xx_mult_neg(ROW_COEF(store.row_update.row, it), 
			       ROW_COEF(L_i, it_i), b);
	  ++it_i;
	  ++it;
	}
	       ...
	       ROW_VAR(*P, it_k) < ROW_VAR(L_i, it_i))
	{
	  ...
	  LAsigned_xx_set(ROW_COEF(store.row_update.row, it), 
			  ROW_COEF(*P, it_k));
      ...
      else
	{
	  ...
	  LAsigned_xx_com(store.row_update.a, 
...
(Tsimplex_var k, Tsimplex_var i, TLAsigned_xx a, TLAsigned_xx b)
{
  ...
  if (offset == ROW_LEN(L_k))
	   ...
	   ++it, ++it_k)
	{
	  ...
	  ROW_ID(L_k, it_k) = k;
	  LAsigned_xx_set(ROW_COEF(L_k, it_k), 
...
linear_expr_update(Tsimplex_var i, TLAsigned_xx a, TLAsigned_xx b)
{
  ...
  if (offset == ROW_LEN(*Prow))
      ...
      while (it_k < ROW_LEN(*Prow) || it_i < ROW_LEN(L_i))
	{
	  if (it_k == ROW_LEN(*Prow) || 
	  ...
	  else
	    {
	      ...
	      LAsigned_xx_set(store.row_update.a,
			      ROW_COEF(*Prow, it_k));
	      LAsigned_xx_com(store.row_update.a, 
			      ROW_COEF(L_i, it_i), a, b);
	      if (!LAsigned_xx_is_zero(store.row_update.a))
	   ...
	   ++it, ++it_k)
	{
	  ...
	  ROW_ID(*Prow, it_k) = LAVAR_UNDEF;
	  LAsigned_xx_set(ROW_COEF(*Prow, it_k), 
			  ROW_COEF(store.row_update.row, it));
	}
      row_reset(store.row_update.row);
    }
  ...
  if (simplex_conflict_direct)
      ...
      while (i < ROW_LEN(row))
	{
	  if (ROW_VAR(row, i) != simplex_conflict_var)
	    {
	      bool opposite = LAsigned_xx_sign_diff(ROW_COEF(row, i),
						    store.conflict.c);
          /* using xor to negate
...
backtrack_bound(Tsimplex_bound * Psimplex_bound,
                Tsimplex_var var, Tlit reason, TLAdelta_xx *bound,
                unsigned char mask)
{
...
static void
backtrack_upper_bound(Tsimplex_var var, Tlit reason, TLAdelta_xx *bound,
                      unsigned char mask)
{
...
static void
backtrack_lower_bound(Tsimplex_var var, Tlit reason, TLAdelta_xx *bound,
                      unsigned char mask)
{
  Tsimplex_bound *Psimplex_bound =
...
{
  LAdelta_xx_set(&simplex_var[Psimplex_bound->var].bound[B],
...
update(Tsimplex_var var, TLAdelta_xx *v)
{
  ...
  if (LAdelta_xx_is_zero(&store.update.D))
    return;
  Pmon = COL(var);
  while (Pmon)
    {
...
pivot_candidate(Tsimplex_var i, Tsimplex_var j,
                TLAsigned_xx ci, TLAsigned_xx cj, bool upp)
{
  if (i == j)
    return false;
  if (LAsigned_xx_sign_diff(ci, cj))
    upp = !upp;
...
static inline void
solve_bound(Tsimplex_var i, /*TLAsigned_xx ci,*/
  ...
  while (1)
    {
      ...
      if (IS_BOUND_LOW(i) &&
        ...
        solve_bound(i, /*store.solve.c,*/ &nb_heuristic_choices, LOW);
               ...
               LAdelta_xx_less(&simplex_var[i].bound[UPP],
...
assert_equality(void)
{
  ...
  LAsigned_xx_set(store.assert_equality.diag, 
  ...
  for (j = 0; j < ROW_LEN(store.constraint_push.row); ++j)
    {
      ...
      ROW_ID(row_v, j) = v;
      LAsigned_xx_set(ROW_COEF(row_v, j), 
...
simplex_xx_constraint_push(unsigned n, Tsimplex_var *vars, TLAsigned_xx *coefs)
{
  ...
  if (!n)
    my_error("simplex_xx_constraint_push: inconsistency\n");
...
simplex_xx_simp_const(void)
{
  ...
  for (var = 1; var < simplex_var_n; ++var)
    {
      ...
      if (PASSIVE(var) || NONBASIC(var) || !simplex_xx_var_fixed(var))
        continue;
  ...
  for (var = 1; var < simplex_var_n; var++)
	...
	Trow row = ROW(var);
        ...
        if (LAdelta_xx_is_zero(&simplex_var[var].assign))
...
{
  stats_unsigned("simplex_var_xx",
                 "Number of variables for simplex (XX version)",
                 "%9u", simplex_var_n);
  stats_unsigned("simplex_var_xx_unbound",
                 "Number of unbounded eliminated variables for simplex (XX version)",
                 "%9u", simplex_var_n_unbound);
  stats_unsigned("simplex_var_xx_const",
                 "Number of constant eliminated variables for simplex (XX version)",
                 "%9u", simplex_var_n_const);
  stats_unsigned("simp_XX_piv",
                 "simplex XX pivoting operations",
                 "%9u", simplex_pivot);
  stats_unsigned("simp_XX_piv_h",
                 "simplex XX pivot heuristics",
  ...
  for (i = 1; i < simplex_var_n; i++)
	...
	for (k = 0; k < ROW_LEN(Li); ++k)
      ...
      if (simplex_var[i].boundmask == 3)
        assert(LAdelta_xx_leq(&simplex_var[i].bound[LOW],
                              &simplex_var[i].bound[UPP]));
      ...
      if (NONBASIC(i) || !simplex_var[i].mark_unsat)
        {
          ...
          if (IS_BOUND_LOW(i))
            assert(LAdelta_xx_leq(&simplex_var[i].bound[LOW],
                                  &simplex_var[i].assign));
          if (IS_BOUND_UPP(i))
            assert(LAdelta_xx_leq(&simplex_var[i].assign,
                                  &simplex_var[i].bound[UPP]));
        }
#ifdef SIMPLEX_COPY
