...
row_free(Trow row)
{
  ...
  unsigned i;
  for (i = 0; i < row->alloc; ++i)
    LAsigned_hw_clear(row->data[i].a);
#endif
  stack_free(row);
}
  ...
  for (i = 0; i < modified_n; i++)
    {
      ...
      LAdelta_hw_set(&simplex_var[modified[i]].assign2,
  ...
  for (i = 0; i < modified_n; i++)
    {
      ...
      LAdelta_hw_set(&simplex_var[modified[i]].assign,
...
static inline void
simplex_var_update(Tsimplex_var i, TLAdelta_hw * D, TLAsigned_hw num, TLAsigned_hw den)
{
  int bound;
...
linear_expr_normalize_hw(Trow row)
{
  ...
  /* compute gcd */
  for (i = 1; LAsigned_hw_notone(store.normalize.tmp) && i < ROW_LEN(row); ++i)
    {
 ...
 const unsigned offset)
{
  ...
  while (it_k < ROW_LEN(*P) || it_i < ROW_LEN(L_i))
	  ...
	  (it_i < ROW_LEN(L_i) && ROW_VAR(L_i, it_i) < ROW_VAR(*P, it_k)))
	{
	  ...
	  LAsigned_hw_mult_neg(ROW_COEF(store.row_update.row, it), 
			       ROW_COEF(L_i, it_i), b);
	  ++it_i;
	  ++it;
	}
	       ...
	       ROW_VAR(*P, it_k) < ROW_VAR(L_i, it_i))
	{
	  ...
	  LAsigned_hw_set(ROW_COEF(store.row_update.row, it), 
			  ROW_COEF(*P, it_k));
      ...
      else
	{
	  ...
	  LAsigned_hw_com(store.row_update.a, 
...
(Tsimplex_var k, Tsimplex_var i, TLAsigned_hw a, TLAsigned_hw b)
{
  ...
  if (offset == ROW_LEN(L_k))
	   ...
	   ++it, ++it_k)
	{
	  ...
	  ROW_ID(L_k, it_k) = k;
	  LAsigned_hw_set(ROW_COEF(L_k, it_k), 
...
linear_expr_update(Tsimplex_var i, TLAsigned_hw a, TLAsigned_hw b)
{
  ...
  if (offset == ROW_LEN(*Prow))
      ...
      while (it_k < ROW_LEN(*Prow) || it_i < ROW_LEN(L_i))
	{
	  if (it_k == ROW_LEN(*Prow) || 
	  ...
	  else
	    {
	      ...
	      LAsigned_hw_set(store.row_update.a,
			      ROW_COEF(*Prow, it_k));
	      LAsigned_hw_com(store.row_update.a, 
			      ROW_COEF(L_i, it_i), a, b);
	      if (!LAsigned_hw_is_zero(store.row_update.a))
	   ...
	   ++it, ++it_k)
	{
	  ...
	  ROW_ID(*Prow, it_k) = LAVAR_UNDEF;
	  LAsigned_hw_set(ROW_COEF(*Prow, it_k), 
			  ROW_COEF(store.row_update.row, it));
	}
      row_reset(store.row_update.row);
    }
  ...
  if (simplex_conflict_direct)
      ...
      while (i < ROW_LEN(row))
	{
	  if (ROW_VAR(row, i) != simplex_conflict_var)
	    {
	      bool opposite = LAsigned_hw_sign_diff(ROW_COEF(row, i),
						    store.conflict.c);
          /* using xor to negate
...
backtrack_bound(Tsimplex_bound * Psimplex_bound,
                Tsimplex_var var, Tlit reason, TLAdelta_hw *bound,
                unsigned char mask)
{
...
static void
backtrack_upper_bound(Tsimplex_var var, Tlit reason, TLAdelta_hw *bound,
                      unsigned char mask)
{
...
static void
backtrack_lower_bound(Tsimplex_var var, Tlit reason, TLAdelta_hw *bound,
                      unsigned char mask)
{
  Tsimplex_bound *Psimplex_bound =
...
{
  LAdelta_hw_set(&simplex_var[Psimplex_bound->var].bound[B],
...
update(Tsimplex_var var, TLAdelta_hw *v)
{
  ...
  if (LAdelta_hw_is_zero(&store.update.D))
    return;
  Pmon = COL(var);
  while (Pmon)
    {
...
pivot_candidate(Tsimplex_var i, Tsimplex_var j,
                TLAsigned_hw ci, TLAsigned_hw cj, bool upp)
{
  if (i == j)
    return false;
  if (LAsigned_hw_sign_diff(ci, cj))
    upp = !upp;
...
static inline void
solve_bound(Tsimplex_var i, /*TLAsigned_hw ci,*/
  ...
  while (1)
    {
      ...
      if (IS_BOUND_LOW(i) &&
        ...
        solve_bound(i, /*store.solve.c,*/ &nb_heuristic_choices, LOW);
               ...
               LAdelta_hw_less(&simplex_var[i].bound[UPP],
...
assert_equality(void)
{
  ...
  LAsigned_hw_set(store.assert_equality.diag, 
  ...
  for (j = 0; j < ROW_LEN(store.constraint_push.row); ++j)
    {
      ...
      ROW_ID(row_v, j) = v;
      LAsigned_hw_set(ROW_COEF(row_v, j), 
...
simplex_hw_constraint_push(unsigned n, Tsimplex_var *vars, TLAsigned_hw *coefs)
{
  ...
  if (!n)
    my_error("simplex_hw_constraint_push: inconsistency\n");
...
simplex_hw_simp_const(void)
{
  ...
  for (var = 1; var < simplex_var_n; ++var)
    {
      ...
      if (PASSIVE(var) || NONBASIC(var) || !simplex_hw_var_fixed(var))
        continue;
  ...
  for (var = 1; var < simplex_var_n; var++)
	...
	Trow row = ROW(var);
        ...
        if (LAdelta_hw_is_zero(&simplex_var[var].assign))
...
{
  stats_unsigned("simplex_var_hw",
                 "Number of variables for simplex (HW version)",
                 "%9u", simplex_var_n);
  stats_unsigned("simplex_var_hw_unbound",
                 "Number of unbounded eliminated variables for simplex (HW version)",
                 "%9u", simplex_var_n_unbound);
  stats_unsigned("simplex_var_hw_const",
                 "Number of constant eliminated variables for simplex (HW version)",
                 "%9u", simplex_var_n_const);
  stats_unsigned("simp_HW_piv",
                 "simplex HW pivoting operations",
                 "%9u", simplex_pivot);
  stats_unsigned("simp_HW_piv_h",
                 "simplex HW pivot heuristics",
  ...
  for (i = 1; i < simplex_var_n; i++)
	...
	for (k = 0; k < ROW_LEN(Li); ++k)
      ...
      if (simplex_var[i].boundmask == 3)
        assert(LAdelta_hw_leq(&simplex_var[i].bound[LOW],
                              &simplex_var[i].bound[UPP]));
      ...
      if (NONBASIC(i) || !simplex_var[i].mark_unsat)
        {
          ...
          if (IS_BOUND_LOW(i))
            assert(LAdelta_hw_leq(&simplex_var[i].bound[LOW],
                                  &simplex_var[i].assign));
          if (IS_BOUND_UPP(i))
            assert(LAdelta_hw_leq(&simplex_var[i].assign,
                                  &simplex_var[i].bound[UPP]));
        }
#ifdef SIMPLEX_COPY
